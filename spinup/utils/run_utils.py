...
def create_grid2op_env(env_name):
    ...
    grid2op_gym.observation_space = BoxGymObsSpace(grid2op_gym.init_env.observation_space,
                                            attr_to_keep=['a_ex', 'a_or', 
                                                          'actual_dispatch', 'attention_budget', 
                                                          'curtailment', 'curtailment_limit', 
                                                          'curtailment_limit_effective', 
                                                          'delta_time', 'gen_margin_down', 
                                                          'gen_margin_up', 'gen_p', 'gen_p_before_curtail', 'gen_q', 
                                                          'gen_theta', 'gen_v', 'load_p', 'load_q', 'load_theta', 'load_v', 
                                                          'p_ex', 'p_or', 'q_ex', 'q_or', 'rho', 
                                                          'target_dispatch', 'thermal_limit', 
                                                          'theta_ex', 'theta_or', 'v_ex', 'v_or'])
    ...
    def thunk_plus():
        ...
        try:
            if 'env_name' in kwargs:
                if kwargs['env_name'] == 'l2rpn_case14_sandbox':
                    import gym
                    env_name = kwargs['env_name']
                    kwargs['env_fn'] = lambda : create_grid2op_env(env_name)
                    del kwargs['env_name']
                else:
                    import gym
                    env_name = kwargs['env_name']
